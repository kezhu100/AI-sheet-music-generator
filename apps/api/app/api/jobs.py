...
from app.models.schemas import CreateJobRequest, JobResponse, JobResult
...
async def export_job_midi(job_id: str) -> Response:
    result = _get_completed_job_result(job_id)
...
def _get_completed_job_result(job_id: str) -> JobResult:
    job = job_store.get(job_id)
    if job is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Job not found.")
